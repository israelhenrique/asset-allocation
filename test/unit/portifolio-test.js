...
describe('Portifolio', () => {
  ...
  describe('#isBalanced', () => {
    it('should return if the portifolio is balanced or not', () => {
      const s1 = new Asset('BOVA11', 1, 80, 52.87, 1)
      const s2 = new Asset('SMAL11', 1, 67, 47.29, 1)
      const s3 = new Asset('FMOF11', 2, 3, 108.00, 1)
      const s4 = new Asset('EURO11', 2, 7, 165.00, 1)
      ...
      const s8 = new Asset('FAED11B', 2, 4, 157.00, 1)
      const s9 = new Asset('HTMX11B', 2, 2, 87.86, 1)
      const s10 = new Asset('RendaFixa', 3, 1, 7270, 1)
      const arrayAcoes = []
      const arrayFII = []
      const arrayRenda = []
      arrayAcoes.push(s1)
      ...
      expect(portifolio.isBalanced()).to.equals('balanced')
      portifolio.alloc = [35, 20, 45]
      expect(portifolio.isBalanced()).to.equals('desbalanced')
    })
  })
})
